...
export class ReservationsController {
  ...
  @UseGuards(JwtAuthGuard)
  @Get('my')
  getMyReservations(@Req() request: Request) {
    const currentUser = request.user as {
      userId: number;
      email: string;
      role: string;
    ...
    return this.reservationsService.getMyReservations(currentUser.userId);
  }
  ...
  @UseGuards(JwtAuthGuard)
  @Get(':id')
  getOne(@Param('id', ParseIntPipe) id: number, @Req() request: Request) {
    const currentUser = request.user as {
      userId: number;
      email: string;
      role: string;
  ...
  @UseGuards(JwtAuthGuard)
  @Patch(':id/cancel')
  cancel(@Param('id', ParseIntPipe) id: number, @Req() request: Request) {
    const currentUser = request.user as {
      userId: number;
      email: string;
      role: string;
